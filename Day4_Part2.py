...
for pair in pass_list:
    if len(pair) == 0:
        ...
        holder.update({pair[:pair.find(':')] : pair[pair.find(':') + 1 : ]})

small_fields = fields[0:3]

hcl = ['g', 'h', 'i', 'j', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
pass_keys = ['byr', 'iyr', 'eyr', 'ecl', 'hcl', 'hgt', 'pid', 'cid']
less_keys = ['byr', 'iyr', 'eyr', 'ecl', 'hcl', 'hgt', 'pid']
valid = 0
for passport in fields:
    print(passport)
    if all(keys in passport for keys in pass_keys):
        if ((len(passport['byr']) == 4) and (int(passport['byr']) >= 1920 and int(passport['byr']) <= 2002)) and ((len(passport['iyr']) == 4) and (int(passport['iyr']) >= 2010 and int(passport['iyr']) <= 2020)) and ((len(passport['eyr']) == 4) and (int(passport['eyr']) >= 2020 and int(passport['eyr']) <= 2030)):
            if (len(passport['ecl']) == 3) and ('gry' or 'amb' or 'blu' or 'brn' or 'grn' or 'hzl' or 'oth' in passport['ecl']):
                if (len(passport['pid']) == 9) and (passport['pid'].isdigit()):
                    if (passport['hcl'][0] == '#') and (len(passport['hcl']) == 7) and (x not in passport['hcl'] for x in hcl):
                        if ('cm' in passport['hgt']) and (len(passport['hgt']) == 5) and ((int(passport['hgt'][:3]) >= 150) and (int(passport['hgt'][:3]) <= 193)):
                            valid += 1
                        elif ('in' in passport['hgt']) and (len(passport['hgt']) == 4) and ((int(passport['hgt'][:2]) >= 59) and (int(passport['hgt'][:2]) <= 76)):
                            valid += 1
    elif all(keys in passport for keys in pass_keys):
        if ((len(passport['byr']) == 4) and (int(passport['byr']) >= 1920 and int(passport['byr']) <= 2002)) and ((len(passport['iyr']) == 4) and (int(passport['iyr']) >= 2010 and int(passport['iyr']) <= 2020)) and ((len(passport['eyr']) == 4) and (int(passport['eyr']) >= 2020 and int(passport['eyr']) <= 2030)):
            if (len(passport['ecl']) == 3) and ('gry' or 'amb' or 'blu' or 'brn' or 'grn' or 'hzl' or 'oth' in passport['ecl']):
                if (len(passport['pid']) == 9) and (passport['pid'].isdigit()):
                    if (passport['hcl'][0] == '#') and (len(passport['hcl']) == 7) and (x not in passport['hcl'] for x in hcl):
